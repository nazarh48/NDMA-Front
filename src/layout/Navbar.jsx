...
export default () => {
  ...
  return (
    <div
      id="sidenav"
      className=" transition-all border-r-2 border-solid border-white duration-300 shadow-xl h-screen bg-components text-white flex flex-col p-2"
    >
      <div>
        <Logo />
      </div>
      ...
      <div className="flex-grow">
        {privateRoutes.map((route, index) => (
          <RouteComponent path={route.path} label={route.label} icon={route.icon} />
        ))}
      </div>
