...
const MouseOverPopover = ({ warehouse }) => {
  ...
  const getWarehouseInfo = async () => {
    try {
      ...
      const response = await _axios("get", `/Warehouse/getWarehouseById/${warehouse.from}`);
                            ...
                            <Grid item md={8}>
                              <Tooltip title="List of warehouses that at least have one item in the inventory">
                                <TextField
                                  ...
                                  {...getFieldProps(`activities[${index}].from`)}
                                  InputLabelProps={{
                                    shrink: p.from ? true : false,
                                  }}
                                  onChange={(e) => {
