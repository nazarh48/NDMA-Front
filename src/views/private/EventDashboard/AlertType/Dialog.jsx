...
const AddUpdateDialog = ({ open, onClose, onEdit, onRefresh, row }) => {
  ...
  const handleSave = async (values) => {
    console.log(values);
    try {
      const response = await _axios("post", `/AlertType/createUpdateAlertType`, values);
      if (response.status === 200) {
        toast.success("Alert Type added successfully");
        onRefresh();
      }
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {onEdit ? "Update Alert Type" : "Add Alert Type"}
        </DialogTitle>
        <DialogContent>
